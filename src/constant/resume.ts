				...
				projects: [
					{
						...
						achievements: [
							{
								title:
									'기관마다 다른 인증·동의·전송요구·전자서명 절차를 단계 단위로 모델링하고, 기관별 단계 구성과 API 요청·응답 처리를 분리해 화면 전환, 진행 상태 저장, 실패 처리를 일관되게 처리했습니다.',
							},
							{
								title:
...
const CAREER_DETAIL_OVERRIDES: Record<string, ResumeItemProps[]> = {
	'마이데이터 연동 및 연결 관리 구축': [
		...
		{
			title:
				'기관마다 다른 인증·동의·전송요구 절차를 단계 단위로 모델링하고, 기관별 단계 구성과 API 요청·응답 처리를 분리했습니다. 사용자가 외부 본인인증 이후 돌아오거나 재진입해도 진행 상태를 복원할 수 있게 했고, 외부 본인인증 SDK는 전용 React 훅으로 분리해 스크립트 로딩, 전자서명 콜백, cleanup을 한 곳에서 관리했습니다.',
		},
		{
			title:
				'기관별로 달랐던 인증·동의·철회 절차를 같은 화면 전환과 예외 처리 기준으로 다룰 수 있게 했습니다. 이후 기관별 보완 요청이 들어와도 화면을 새로 만드는 대신 단계 구성과 API 연결 단위로 대응할 수 있는 프론트엔드 구조를 확보했습니다.',
		},
	],
