				...
				projects: [
					{
						...
						achievements: [
							{
								title:
									'기관별로 다른 인증·동의·전송요구·전자서명 단계를 공통 스텝 구조로 모델링하고, 사용자가 이탈 후 돌아와도 진행 중인 단계에서 이어갈 수 있도록 구성했습니다.',
							},
							{
								title:
					...
					},
					{
						...
						achievements: [
							...
							{
								title:
									'주문서에 표시되는 금액과 PG 결제 요청 금액이 어긋나지 않도록 계산 기준과 예외 상태를 검증하고, NICE Payments 승인·return URL 흐름을 연결해 오픈 이후 결제 금액 불일치 이슈 0건으로 운영했습니다.',
							},
							{
								title:
...
const CAREER_DETAIL_OVERRIDES: Record<string, ResumeItemProps[]> = {
	'마이데이터 연동 및 연결 관리 구축': [
		...
		{
			title:
				'기관마다 다른 인증·동의 절차를 공통 스텝 구조로 모델링하고, 사용자가 이탈하거나 뒤로 가도 진행 중인 단계에서 이어갈 수 있게 구성했습니다. 외부 본인인증 SDK는 전용 React 훅으로 분리해 스크립트 로딩, 전자서명 콜백, cleanup을 한 곳에서 관리했습니다.',
		},
		{
			title:
				'기관별로 달랐던 인증·동의·철회 절차를 같은 화면 구조와 예외 처리 기준으로 다룰 수 있게 했습니다. 이후 기관별 보완 요청이 들어와도 화면을 새로 만드는 대신 스텝 설정과 API 연결 단위로 대응할 수 있는 프론트엔드 구조를 확보했습니다.',
		},
	],
