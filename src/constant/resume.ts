...
const CAREER_DETAIL_OVERRIDES: Record<string, ResumeItemProps[]> = {
	'마이데이터 연동 및 연결 관리 구축': [
		...
		{
			title:
				'그 결과 외부 본인인증 콜백이 필요한 기관과 필요하지 않은 기관을 같은 화면 구조에서 처리하고, 기관별 보완 요청도 단계 구성과 요청·응답 매핑 수정으로 대응할 수 있었습니다. 일부 기관 철회 실패는 실패 건만 분리해 안내해 성공한 연결 상태가 실패처럼 보이지 않도록 처리했습니다.',
		},
	],
