				...
				projects: [
					{
						...
						achievements: [
							{
								title:
									'기관마다 필요한 인증·동의·전송요구·전자서명 단계가 달라, 기관별 어댑터가 필요한 단계와 API 요청·응답 처리를 제공하도록 분리하고 화면 전환·진행 상태 저장·실패 처리는 공통 처리했습니다.',
							},
							{
								title:
...
const CAREER_DETAIL_OVERRIDES: Record<string, ResumeItemProps[]> = {
	'마이데이터 연동 및 연결 관리 구축': [
		...
		{
			title:
				'기관별 어댑터가 해당 기관에 필요한 단계와 각 단계의 요청 생성, 응답 정규화, 에러 매핑을 제공하도록 분리했습니다. 예를 들어 의료 기관은 외부 본인인증 이동과 콜백 검증 단계를 포함하고, 코스콤은 해당 단계 없이 동의·전자서명·전송요구 단계로 진행하도록 구성했습니다.',
		},
		{
			title:
				'공통 로직은 어댑터가 제공한 단계를 실행하고 진행 상태 저장, 화면 전환, 실패 처리를 담당했습니다. 외부 인증 이후 콜백으로 돌아오면 저장된 기관·단계 정보를 검증해 이어서 처리했고, 일부 기관 철회 실패는 실패 건만 분리해 안내했습니다.',
		},
	],
