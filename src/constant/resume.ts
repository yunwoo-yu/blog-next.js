				...
				projects: [
					...
					},
					{
						...
						details: [
							...
							{
								title:
									'NexBe 본인인증 SDK의 스크립트 로딩과 콜백 순서를 커스텀 훅으로 감싸고, KHIS·KOSCOM별 인증·동의·전송요구·전자서명 화면을 구현했습니다. 연동 중간값은 세션스토리지에 단계별로 저장·정리하고, 연결 관리에서는 분야별 상세·재동의·개별·일괄 철회·전송 이력을 구현했습니다.',
								highlights: [
									'스크립트 로딩과 콜백 순서를 커스텀 훅으로 감싸고',
									'연동 중간값은 세션스토리지에 단계별로 저장·정리',
								],
							},
							{
								title:
				...
				projects: [
					{
						...
						details: [
							...
							{
								title:
									'Next와 Vue의 응답 인터셉터에 같은 401 복구 절차를 적용했습니다. refresh Promise를 공유해 첫 요청만 토큰을 갱신하고 나머지는 같은 결과를 기다리게 했습니다. 실패 요청과 쿠키 토큰이 다르면 다른 앱이 이미 갱신한 것으로 보고 최신 토큰으로 재시도했습니다.',
								highlights: ['첫 요청만 토큰을 갱신하고 나머지는 같은 결과를 기다리게', '최신 토큰으로 재시도'],
							},
							{
								title:
									'Next와 Vue가 하나의 갱신 결과와 쿠키를 사용해 반복 401과 중복 refresh를 줄였습니다. 토큰 갱신 실패 때만 공통 만료 처리로 이동해 동시 요청으로 운영 작업이 끊기지 않게 했습니다.',
				...
				projects: [
					...
					},
					{
						...
						details: [
							...
							{
								title:
									'사용자 서비스를 Next.js 15 App Router로 전환하고, 고객사 코드로 조회한 설정을 서버에서 메타데이터·OpenGraph에 반영했습니다. 사이트맵도 고객사 코드 목록 API를 기준으로 생성해 새 고객사의 검색 경로가 자동으로 포함되게 했습니다.',
								highlights: [
									'고객사 코드로 조회한 설정을 서버에서 메타데이터·OpenGraph에 반영',
									'고객사 코드 목록 API를 기준으로 생성',
								],
							},
							{
								title:
									'고객사별 제목·설명·다국어 OpenGraph와 사이트맵을 운영 데이터로 관리하게 해, 프론트엔드 배포 없이 검색 노출 정보를 변경하거나 신규 고객사를 추가할 수 있는 구조로 전환했습니다.',
