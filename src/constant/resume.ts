				...
				projects: [
					{
						...
						details: [
							{
								title:
									"고향사랑기부 답례품 기반 지역 상품을 판매하는 커머스 '웰마켙'을 오픈하면서, 주문서에 표시되는 금액과 실제 PG 결제 요청 금액이 같은 기준으로 계산되어야 했습니다. 상품 금액, 배송비, 쿠폰, 웰포인트가 함께 반영되는 구조라 결제 전 금액 계산과 예외 상태를 화면에서 먼저 검증해야 했습니다.",
							},
							{
								title:
									'주문 금액 구성과 결제수단을 단일 체크아웃 상태로 관리하고, 결제 금액은 체크아웃 API 반환값을 NicePay SDK에 그대로 전달해 화면 표시 금액과의 불일치를 차단했습니다. 결제 전 배송지·결제수단·약관을 순서대로 검증하고, 공식 매뉴얼 기준으로 인증 결과 코드별(사용자 취소·통신 실패 등) 안내를 분리했으며, 재고 소진·주문 만료는 재고 모달·장바구니 복귀로 복구되게 했습니다.',
							},
							{
								title:
									'주문서부터 결제 승인·배송·반품·환불·어드민 연결까지 커머스 주문 플로우 전체를 설계·구현해 오픈했습니다. 오픈 후 한 달간 결제 금액 관련 이슈 0건으로 운영했고, 사용자·운영팀·입점업체가 같은 주문 상태 기준으로 배송 처리, 반품·환불, 고객 응대를 이어갈 수 있는 기반을 만들었습니다.',
							},
						],
					},
					{
						...
						details: [
							...
							{
								title:
									'모바일 전용이던 화면들을 PC까지 확장하고, 흩어진 콘텐츠를 딥링크 가능한 통합검색으로 묶었습니다. 개편 전후 2주간 GA4 데이터 비교 기준으로 desktop 평균 참여 시간이 약 20% 증가했습니다.',
							},
						],
					},
					{
						...
						details: [
							...
							{
								title:
									'외부 지로 납부 화면을 거쳐도 Wello에서 결제 진행 상태를 다시 확인할 수 있게 하면서, 배포 이후 중복 기부 여부를 묻는 반복 문의가 거의 들어오지 않게 됐습니다. 납부 화면 이동과 복귀 처리는 실행 환경별로 분기해 앱·웹뷰에서도 같은 기준으로 완료 여부를 확인하도록 했습니다.',
							},
						],
					},
					{
						...
						details: [
							...
							{
								title:
									'기관마다 단계 구성과 API 처리 방식이 달라 어댑터 패턴을 적용했습니다. 기관별로 필요한 단계, 요청 생성, 응답 정규화, 에러 매핑을 어댑터로 분리하고, 공통 로직은 단계 실행, 진행 상태 저장, 화면 전환, 실패 처리만 담당하게 했습니다. 스텝 식별자는 URL 쿼리스트링에 저장해 새로고침 시 진행 상태를 복원하고, 인증 중간값은 탭 종료 시 자동 소멸되는 세션스토리지에 분리해 민감값이 남지 않게 했습니다.',
							},
							{
								title:
									'의료·통신 기관과 NICE 신용정보 연동 플로우를 구현해 심사를 진행했고, 심사 과정의 기관별 보완 요청을 화면을 새로 만들지 않고 어댑터 단위 수정만으로 대응했습니다. 새 기관이 추가되더라도 기존 화면과 공통 플로우의 변경 범위를 줄일 수 있는 구조를 확보했습니다.',
							},
						],
					},
				],
				...
				projects: [
					{
						...
						details: [
							...
							{
								title:
									'한글 IME 조합 중 완성 전 문자열이 조건으로 추가되던 문제를 해결하고, OR·AND·NOT 키워드 조건을 11개 입력 지점에서 같은 추가·삭제·초기화 동작과 검증 기준으로 다룰 수 있게 했습니다.',
							},
						],
					},
				],
				...
				projects: [
					{
						...
						details: [
							...
							{
								title:
									'OpenAI Assistants API의 Threads·Messages·Runs 흐름을 서버 SSE 응답으로 전달해 답변 생성 과정을 실시간으로 보여주고, 역방향 무한 스크롤로 채팅 히스토리 조회 UX를 개선했습니다. CMS에서는 파일 등록, 벡터 스토어 관리, AI 콘텐츠 자동 생성, 통계 대시보드, 프롬프트·모델 설정 화면을 구현했습니다.',
								links: [
									{
										title: '관련글',
							...
							{
								title:
									'CSR 기반 React 서비스를 Next.js 15 App Router SSR 구조로 전환하고, 고객사별 동적 메타데이터·다국어 OpenGraph·API 기반 사이트맵을 적용해 검색 노출 관리를 자동화했습니다. 고객사는 챗봇 응대와 CMS 운영을 직접 관리할 수 있게 되었고, 무료 플랜에는 응답 횟수 기반 광고 삽입을 적용해 라이브로 배포·운영했습니다.',
							},
						],
					},
					{
				...
				projects: [
					{
						...
						details: [
							...
							{
								title:
									'프론트엔드 영역을 단독으로 설계·개발해 웹 서비스와 CMS 운영 기능을 실제 운영 가능한 상태로 완성했습니다. 직접 구축한 보일러플레이트와 CMS UI Kit을 실제 서비스에 처음 적용해, 공통 기반의 실효성을 검증한 프로젝트였습니다.',
							},
						],
					},
				],
				...
				projects: [
					{
						...
						details: [
							...
							{
								title:
									'Yarn Berry 모노레포와 Lerna 버전 관리 구조로 UI 라이브러리, 설정, 유틸 공통 패키지를 함께 관리했습니다. Swing 서비스 웹·CMS에 적용해 초기 세팅 시간을 줄였고, 버튼·폼·테이블·레이아웃 등 반복 UI를 일관된 기준으로 재사용했습니다.',
							},
						],
					},
				],
				...
				projects: [
					{
						...
						details: [
							...
							{
								title:
									'3초 이상 지연되던 첫 화면 표시를 400~600ms 수준(Lighthouse FCP 기준)으로 단축하고, 거대 컴포넌트를 공통 컴포넌트와 커스텀 훅으로 분리해 레거시에 큰 영향 없이 기능을 수정할 수 있는 구조를 만들었습니다.',
							},
						],
					},
				],
...
const ACTIVITIES: Activity[] = [
	{
		...
		items: [
			...
			'클린 코드 리팩토링과 SRP/FSD 설계를 적용하며 유지보수 가능한 구조를 연습했습니다.',
		],
	},
	{
