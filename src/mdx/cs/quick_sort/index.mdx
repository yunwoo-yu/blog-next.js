...
function quickSort(arr) {
  ...
  return [...quickSort(minArray), ...equalArray, ...quickSort(maxArray)];
}

console.log(quickSort(array)); // [1, 2, 2, 3, 4, 5]
```

js 메서드인 filter를 이용해 가운데 위치의 값을 pivot으로 잡고, 작은값을 모은 배열과 큰값을 모은 배열을 합치는 과정을 재귀적으로 반복합니다.

pivot과 **같은 값을 모으는 배열이 따로 필요합니다.** `<`와 `>`로만 나누면 pivot과 값이 같은 요소들이 양쪽 어디에도 들어가지 못하는데, 마지막에 pivot 하나만 끼워 넣으면 나머지가 사라지기 때문입니다. `[2, 2, 1]`을 넣으면 `[1, 2]`가 나오는 식입니다.
...
function quickSort(arr, left = 0, right = arr.length - 1) {
  ...
  while (minTarget <= maxTarget) {
    ...
    // 피벗 이상인 값을 만날 때까지 minTarget 포인터를 오른쪽으로 이동
    while (arr[minTarget] < pivotValue) {
      minTarget++;
    }

    // 피벗 이하인 값을 만날 때까지 maxTarget 포인터를 왼쪽으로 이동
...
두 포인터를 이동시키는 조건이 `<`, `>` 라서 pivot과 값이 같은 요소를 만나면 멈춥니다. 그 자리에서 교환이 일어나기 때문에 같은 값들이 양쪽으로 나뉘어 흩어지고, 덕분에 중복이 많은 배열에서도 분할이 한쪽으로 쏠리지 않습니다.
