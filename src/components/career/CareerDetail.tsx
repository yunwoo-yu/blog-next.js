			...
			{RESUME.careers.map(career => (
				<section key={career.organization}>
					...
					{career.serviceGroups.map(group => (
						<div
							key={group.service}
							className={`mb-8 flex flex-col gap-8 print:mb-4 print:gap-4 ${group.printBreakBefore ? 'print:break-before-page' : ''}`}>
							<div className="mt-2 flex items-baseline gap-2 print:mt-3">
								<h3 className="text-lg font-medium text-destructive">{group.service}</h3>
							...
							{group.projects.map(project => (
								<article
									...
									<div className="mb-3 print:mb-2">
										<h4 className="text-xl font-medium print:text-lg">{project.title}</h4>
